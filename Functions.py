...
def bollinger():
    ...
    for stockkey in STI_dict:
        try:
            ...
            st.write('error: incomplete list')
...
def rsical():
    ...
    for stockkey in STI_dict:
        try:
            ...
            if latest_rsi > 70:
                selltext = f"""\n {stockkey} - {STI_dict[stockkey][0]} [chart]({STI_dict[stockkey][1]}) | Latest RSI: {latest_rsi}"""
                sell2 = write_file(selltext, filepath='currentsell.txt')

            elif latest_rsi < 30:
                buytext = f"""\n {stockkey} - {STI_dict[stockkey][0]} [chart]({STI_dict[stockkey][1]}) | RSI: {latest_rsi}"""
                buy2 = write_file(buytext, filepath='currentbuy.txt')
